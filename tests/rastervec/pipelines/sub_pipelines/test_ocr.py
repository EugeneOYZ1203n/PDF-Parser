...
def test_recognize_unique_words_delegates_to_recognize_segments(monkeypatch):
    captured: dict = {}
    ...
    uniques = [Segment(vectors=[], angle=0.0), Segment(vectors=[], angle=0.0)]

    result = ocr_mod.recognize_unique_words(uniques)

    assert result == ["text for 2 word(s)"]
    assert captured["calls"] == [(uniques, None)]


def test_recognize_unique_words_empty_input():
    assert ocr_mod.recognize_unique_words([]) == []
...
def test_recognize_unique_words_passes_recognize_fn_only_when_compute_given(monkeypatch):
    captured: list = []
    ...
    ocr_mod.recognize_unique_words([])
    assert captured[-1] is None

    ocr_mod.recognize_unique_words([], compute=_FakeCompute())
...
def test_recognize_fn_closure_dispatches_through_compute_apply(monkeypatch):
    ...
    ocr_mod.recognize_unique_words([], compute=compute)
...
def test_recognize_fn_closure_increments_progress_counter_by_crop_count(monkeypatch):
    ...
    ocr_mod.recognize_unique_words([], compute=_FakeCompute(), progress_counter=counter)
...
def test_recognize_unique_words_no_progress_counter_without_compute(monkeypatch):
    ...
    monkeypatch.setattr(ocr_mod, "_recognize_segments", fake_recognize_segments)
    ocr_mod.recognize_unique_words([], progress_counter=_FakeCounter())
    assert captured[-1] is None
...
# restore_word_texts
# --------------------------------------------------------------------------
def test_restore_word_texts_transforms_canonical_text_by_meta(text):
    ...
    [restored] = ocr_mod.restore_word_texts([canonical], [meta])
...
def test_restore_word_texts_identity_transform_leaves_geometry_unchanged(text):
...
def test_restore_word_texts_duplicates_one_unique_across_many_metas(text):
    ...
    restored = ocr_mod.restore_word_texts([canonical], metas)
...
def test_restore_word_texts_selects_by_unique_index(text):
    """Two different representatives' texts are placed onto the right
    occurrences via `meta.unique_index`, not positionally."""
    word_a = text(text="A", bbox=(0.0, 0.0, 5.0, 5.0), direction=(1.0, 0.0))
    word_b = text(text="B", bbox=(0.0, 0.0, 5.0, 5.0), direction=(1.0, 0.0))
    metas = [
        SegmentMeta(unique_index=1, offset=(0.0, 0.0), rotation=0.0, page_index=0, seqno=0),
        SegmentMeta(unique_index=0, offset=(100.0, 0.0), rotation=0.0, page_index=0, seqno=1),
    ]

    restored = ocr_mod.restore_word_texts([word_a, word_b], metas)

    assert [t.text for t in restored] == ["B", "A"]
    assert restored[1].bbox == (100.0, 0.0, 105.0, 5.0)
