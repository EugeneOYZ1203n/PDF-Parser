...
def extract_paths(page: Page) -> list[VectorPath]:
    ...
    paths = [p for record in extract_records(page) for p in record.items]
    _LOG.debug(
...
def extract_records(page: Page) -> list[VectorRecord]:
    ...
    for seq, drawing in enumerate(fitz_page.get_drawings()):
        ...
        for item_index, item in enumerate(drawing.get("items", [])):
            path = _extract_item(item, item_index, common)
            if path is not None:
                items.append(path)
...
def _extract_item(item: tuple, item_index: int, common: dict) -> VectorPath | None:
    op = item[0]
    if op == "l":
        return _extract_line(item, item_index, common)
    if op == "re":
        return _extract_rect(item, item_index, common)
    if op == "qu":
        return _extract_quad(item, item_index, common)
    if op == "c":
        return _extract_curve(item, item_index, common)
    return None


def _extract_line(item: tuple, item_index: int, common: dict) -> VectorPath:
...
def _extract_rect(item: tuple, item_index: int, common: dict) -> VectorPath:
...
def _extract_quad(item: tuple, item_index: int, common: dict) -> VectorPath:
...
def _extract_curve(item: tuple, item_index: int, common: dict) -> VectorPath:
