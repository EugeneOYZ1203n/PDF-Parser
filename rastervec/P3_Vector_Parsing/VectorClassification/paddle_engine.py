...
This backend's OCR is recognition-only: there is no `PaddleDetectBackend`/
`detect_on_cluster` here (unlike `FastIntoPaddle`'s own `paddle_engine.py`)
-- text candidates come from classification + FAST + Radon segmentation
instead, so PaddleOCR's own detector is never invoked in this pipeline.
"""
...
from rastervec.commons.models import Segment, Text
...
def _normalize_bgr(crop: np.ndarray) -> np.ndarray:
    ...
    the crop already carries its white margin from `radon.py`'s own
    `commons.renderer.ocr_prep.pad_image_uniform` calls, and
    `text_recognizer` resizes to its own `rec_image_shape` internally, so
    there is no separate crop-normalization pass to run."""
