...
# Back-compat alias -- kept for notebooks/skew_method_comparison.ipynb's own
# `R._rotation(...)` calls, written before this was made public for reuse
# by `pipelines/_steps.py::segment_paddle_detections`.
_rotation = rotation_transform
...
def assign_vectors_to_segments(
...
def segment_clusters(
    ...
    bbox overlap, see `assign_vectors_to_segments`) -> grow each kept
    ...
    for cluster in clusters:
        ...
        assignments = assign_vectors_to_segments(cluster, seg_bboxes)
...
# --------------------------------------------------------------------------
# `fast_paddle` pipeline's segmentation step: a scoped variant of
# `segment_clusters` above, for one already-known single-line PaddleOCR
# detection box instead of a whole (possibly multi-line) FAST-surviving
# cluster. Crops directly out of an already-rendered cluster image (no
# re-render) and skips `line_bands` entirely (the detection box already
# bounds one line) -- only the word/char gap split (`group_by_aspect`) is
# reused. Rotation comes from the detection's own `rotation_deg`
# (PaddleOCR's placeholder estimate -- see `OCR/Paddle_OCR/ocr_backend.py::
# PaddleDetection`), not a fresh Radon sweep.
# --------------------------------------------------------------------------
def segment_paddle_detections(
    ...
    *,
    ...
    """`cluster_image`/`cluster_dpi` are the **one** render
    `PaddleDetectBackend.detect_on_cluster` already made of
    `cluster_vectors` (`ClusterDetection.image`/`.dpi`) -- nothing here
    re-renders from vectors. `padding` must be the exact same value that
    render was made with (`pipelines/_steps.py::_cluster_render_padding`)
    -- every `page_points_to_pixel`/`pixel_to_page_bbox` call below inverts
    that specific render, and a mismatched `padding` would drift every
    mapped coordinate by `padding * zoom` px. `detections` are that
    cluster's own PaddleOCR-detected text boxes (page space);
    `assigned_vectors[i]` is `detections[i]`'s own vectors, already
    partitioned by `pipelines/_steps.py::reassign_by_overlap` (a vector
    belongs to at most one detection). For each detection with at least
    one assigned vector: rotate the whole cluster image upright by that
    detection's own `rotation_deg` (`rotation_transform`, the same
    forward/inverse construction `segment_clusters` uses above), crop to
    that detection's own quad region, then split that single-line crop
    into word/character segments purely by ink-column gaps
    (`group_by_aspect`) -- no line-band search. Detections with zero
    assigned vectors are skipped (already dropped upstream by
    `reassign_by_overlap`; defensive here too)."""
    segments: list[Segment] = []
    gray_full = to_gray(cluster_image[..., ::-1])  # BGR -> RGB for luminosity

    for detection, det_vectors in zip(detections, assigned_vectors):
        if not det_vectors:
            continue

        out_shape, forward, inverse = rotation_transform(gray_full.shape, detection.rotation_deg)
        rotated = warp(
            gray_full, forward.inverse, output_shape=out_shape,
            cval=255.0, order=1, preserve_range=True,
        ).astype(np.uint8)
        height, width = rotated.shape

        quad_px = page_points_to_pixel(cluster_vectors, cluster_dpi, detection.quad, padding)
        rotated_quad = forward(np.asarray(quad_px, dtype=np.float64))
        rx0 = max(0, int(math.floor(float(rotated_quad[:, 0].min()))))
        rx1 = min(width, int(math.ceil(float(rotated_quad[:, 0].max()))))
        ry0 = max(0, int(math.floor(float(rotated_quad[:, 1].min()))))
        ry1 = min(height, int(math.ceil(float(rotated_quad[:, 1].max()))))
        if rx1 <= rx0 or ry1 <= ry0:
            continue
        line = rotated[ry0:ry1, rx0:rx1]
        ink = to_ink(line)
        if not ink.any():
            continue

        gap_threshold = _cluster_gap_threshold(_line_gaps(ink.any(axis=0)))
        word_boxes = group_by_aspect(line, gap_threshold=gap_threshold, pad=0)
        if not word_boxes:
            continue
        ...
        for wx0, wy0, wx1, wy1 in word_boxes:
            box_rotated = np.array(
                [(rx0 + wx0, ry0 + wy0), (rx0 + wx1, ry0 + wy0),
                 (rx0 + wx1, ry0 + wy1), (rx0 + wx0, ry0 + wy1)], dtype=np.float64,
            )
            box_original_px = inverse(box_rotated)
            seg_bboxes.append(
                pixel_to_page_bbox(cluster_vectors, cluster_dpi, box_original_px.tolist(), padding)
            )

        assignments = assign_vectors_to_segments(det_vectors, seg_bboxes)
        for seg_vectors, tight_bbox in zip(assignments, seg_bboxes):
            if not seg_vectors:
                continue
            grown_bbox = union_bbox([tight_bbox] + [v.bbox for v in seg_vectors])
            gpx0, gpy0, gpx1, gpy1 = grown_bbox
            corners_page = [(gpx0, gpy0), (gpx1, gpy0), (gpx1, gpy1), (gpx0, gpy1)]
            corners_original_px = page_points_to_pixel(cluster_vectors, cluster_dpi, corners_page, padding)
            corners_rotated = forward(np.asarray(corners_original_px, dtype=np.float64))
            ggx0 = max(0, int(math.floor(float(corners_rotated[:, 0].min()))))
            ggx1 = min(width, int(math.ceil(float(corners_rotated[:, 0].max()))))
            ggy0 = max(0, int(math.floor(float(corners_rotated[:, 1].min()))))
