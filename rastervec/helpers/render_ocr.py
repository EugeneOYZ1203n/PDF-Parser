"""RenderOCR helper: render + OCR, backend-agnostic (see helpers/
ocr_backend.py for the pluggable OcrBackend strategy pattern -- PaddleOCR
and Tesseract implementations, both returning detected boxes already
mapped into the caller's original image pixel space).
...
from rastervec.helpers.ocr_backend import OcrBackend, PaddleOcrBackend
from rastervec.models import OcrWord, Page, RasterImage, TextVectorResult, VectorPath
...
# PaddleOCR's/Tesseract's detectors both do noticeably worse on tiny crops
# (e.g. a single narrow dimension line's short text), so a small cluster's
# own bbox shouldn't be allowed to starve it of resolution.
MIN_RENDER_SIDE_PX = 50


class RenderOCR:
    """Render + detect + confidence-voting, shared by the vector-text and
    raster-image OCR steps. `backend` defaults to PaddleOcrBackend --
    pass `backend=TesseractOcrBackend()` (or any other OcrBackend) to swap
    engines without touching this class."""
    ...
    def ocr_cluster(
        ...
        """Shared entrypoint: render (via Renderer) once, upright, and run
        one backend.detect() call over it -- for either a vector path
        cluster (list[VectorPath], via Renderer.render_vector_cluster --
        the only case actually usable today) or a raster image region
        (RasterImage, via Renderer.render_raster_region, which still
        raises NotImplementedError until the Raster stage is built).
        `rotation_used` comes from the backend's own detected orientation,
        not a manual rotation search. `words` is one OcrWord per detected
        box, each mapped from pixel space to page space via
        ...
        cluster bbox (only meaningful for vector-path clusters; a
        RasterImage has no pixel-to-page-space mapping defined yet).
        `ocr_bbox` is the union of every detected box, in page space
        (only when the input was a vector-path list and something was
        detected). For a vector-path cluster, `dpi` is bumped upward (never
        down) if needed so the rendered image's shorter side is at least
        MIN_RENDER_SIDE_PX -- a small cluster's own bbox otherwise renders
        to a handful of pixels at the default dpi, which both OCR backends
        read poorly. The same (possibly bumped) dpi is reused below for
        pixel_to_page_bbox, so the pixel<->page-space mapping always
        matches the image actually rendered."""
        ...
        return TextVectorResult(
            paths=cluster if isinstance(cluster, list) else [],
            text=text,
