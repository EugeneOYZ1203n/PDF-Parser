...
from rastervec.commons.renderer.stages import render_fast  # noqa: F401 -- re-exported for callers
...
class FastDetector:
    ...
    def detect_tiled(
        ...
        """Runs FAST over `image` (rendered by the caller at whatever
        resolution FAST actually needs -- see the module-level note below;
        `scale` upscales further only if a caller still wants that, default
        1.0 does nothing) split into `block_size`-square tiles spaced
        `stride = block_size * (1 - overlap)` apart -- adjacent tiles
        overlap by `overlap` of their own size, and the last tile in each
        row/column is shifted to align exactly with the far edge instead of
        being padded (padding is only ever needed when `image` itself is
        smaller than one `block_size` tile). Each tile is detected once (no
        rotation sweep). Every tile's mask is stitched back into one full
        mask at the *scaled* resolution by taking the **max** score
        wherever tiles overlap (so a line of text that would otherwise
        straddle a tile boundary is fully covered by at least one tile,
        instead of being split between two independent detections), then
        resized back down to `image`'s own original size before returning
        -- callers sample it exactly like `detect()`'s own return value, at
        `image`'s own pixel coordinates. `show_progress` wraps the local
        (`compute is None`, `progress_counter is None`) block loop in a
        `tqdm` bar (`desc`), since a large page can mean hundreds of tiles
        -- this is the fallback for a bare call with no outer progress bar
        to report into (e.g. a notebook running one page directly).

        **Scaling note (fixed to be dpi-based, not resize-based):** callers
        used to render at a low dpi and pass a real `scale` here to
        upscale via `Image.resize(..., Image.BICUBIC)` before tiling --
        raster-upscaling a low-dpi render is blurrier than rendering
        natively at the target resolution. Callers now render directly at
        the combined effective dpi (e.g. `FAST_PAGE_RENDER_DPI *
        FAST_TILE_SCALE_FACTOR`) via `renderer.render_page_paths` and pass
        `scale=1.0` (the default) here, so no PIL resize ever happens on
